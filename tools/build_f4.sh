#!/usr/bin/env bash
#
# Build script for STM32F4 Motor Control firmware
# Updated for modular directory structure:
# boot/f4/, src/f4/, drivers/, middleware/, etc.
#
...
"$CC" "${CPU_FLAGS[@]}" "-Tboot/f4/stm.ld" "-Wl,--gc-sections,--print-memory-usage,-Map=$MAP_FILE" "${objects[@]}" -lc -lm -o "$ELF"
echo "[OK] Linked: $ELF"
